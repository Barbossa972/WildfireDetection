...
def add_color(X, colors_dict = {0 : [0,0,0], 1 : [0, 100, 0], 2 : [0, 0, 100], 3 : [100, 0, 0]}):
    label = torch.zeros((len(colors_dict)))
    color_id = torch.multinomial(torch.Tensor(list(colors_dict.keys())), 1, replacement=True)
    X = torch.remainder(X + torch.Tensor(colors_dict[color_id.item()]).view(3,1,1), 255)
    label[color_id] = 1
    return X, label

if __name__ == '__main__':

    X = torch.rand(3,25,25)
    X_colored, id = add_color(X)
    print(X_colored - X, id)